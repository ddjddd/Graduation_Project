...
# Base Neural Network 정의
# Resnet 사용
shared_layers = nn.nn_base(img_input, trainable=True)

# RPN  정의, 베이스 레이어 삽입
# RPN : Feature map 에서 도출(제안)된 관심 영역
num_anchors = len(C.anchor_box_scales) * len(C.anchor_box_ratios)
rpn = nn.rpn(shared_layers, num_anchors)

# rpn_classifier : 물체일 확률 도출
classifier = nn.classifier(shared_layers, roi_input, C.num_rois, nb_classes=len(classes_count), trainable=True)

# model_rpn : Feature map 으로부터 관심영역을 도출하는 NN 레이어
model_rpn = Model(img_input, rpn[:2])
# model_classifier : Feature map 및 관심 영역을 입력으로 하여 영역 내의 이미지를 분류하는 분류기
model_classifier = Model([img_input, roi_input], classifier)

# 본 모델의 전체적 구성은 RPN 도출 네트워크와 물체 분류 네트워크의 융합
...
for epoch_num in range(num_epochs):
    ...
    while True:
        try:
            ...
            # 결과값 저장을 위한 progress bar update 과정
            ...
            if iter_num == epoch_length:
                ...
                # 생성된 새로운 loss 결과 값이 기존의 최상의 loss 결과 값보다 낮은 경우에만 업데이트 진행
